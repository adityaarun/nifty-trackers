...
stocks_list = ["CRUD", "GOLD"]
client = MongoClient('localhost', 27017)
stocks_db = client['iisc']
commodities_actual = stocks_db['commodity-actual']

for commodity in stocks_list:
    print(commodity)
    price_data = get_stock_data(commodity, start_date, end_date)
    price_data['Symbol'] = commodity
    json_data = json.loads(price_data.to_json(orient='records'))
    commodities_actual.insert_many(json_data)
